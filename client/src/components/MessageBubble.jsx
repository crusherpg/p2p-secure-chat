...
const MessageBubble = ({ message, isOwn, onDecrypt }) => {
  ...
  useEffect(() => {
    if (message.type === 'text' && message.content?.encrypted && onDecrypt) {
      decryptMessage();
    }
  }, [message.content, onDecrypt]);
              ...
              {Array.from({ length: 20 }, (_, i) => (
                <div
                  ...
                  className={`w-1 bg-current rounded-full transition-all duration-150 ${
                    isPlaying && i <= 8 ? 'h-6' : 'h-2'
                  }`}
                  style={{
                    opacity: isPlaying && i <= 8 ? 1 : 0.5,
  ...
  return (
    <>
      <div className={`flex message-slide-in ${
        ...
        <div className={`max-w-xs sm:max-w-sm md:max-w-md lg:max-w-lg xl:max-w-xl message-bubble ${
          isOwn
            ...
            : 'bg-white dark:bg-dark-700 text-gray-900 dark:text-white rounded-r-2xl rounded-tl-2xl rounded-bl-md shadow-sm'
        } p-3 relative`}>
          ...
          {/* Message Metadata */}
          <div className={`flex items-center justify-end space-x-1 text-xs mt-2 ${
