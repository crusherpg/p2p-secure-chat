...
const Emoji = ({ onPick }) => {
  ...
  return (
    <div className="absolute bottom-12 left-16 bg-white border border-gray-200 rounded-lg p-2 shadow-lg grid grid-cols-6 gap-1 z-10">
      {emojis.map(e => (
        <button key={e} className="text-xl hover:bg-gray-100 rounded p-1" onClick={() => onPick(e)}>
          {e}
        </button>
      ))}
    </div>
    ...
    <div className="flex-1 overflow-y-auto">
      ...
      {users.map(u => (
        <button 
          key={u.id} 
          onClick={() => onSelect(u)} 
          className={`user-item ${activeId === u.id ? 'bg-gray-50' : ''}`}
        >
          <div className="avatar">
            {u.avatar ? (
              <img src={u.avatar} alt={u.name} className="w-full h-full object-cover" />
            ) : (
              (u.username?.[0] || u.name?.[0] || 'U')
            )}
          </div>
...
const ChatPage = ({ onOpenSettings }) => {
  ...
  useEffect(() => {
    ...
    if (saved) {
      try {
        ...
        console.warn('Failed to parse saved active conversation:', e);
      }
    }
  }, []);
  ...
  useEffect(() => {
    if (active) {
      localStorage.setItem('p2p_active', JSON.stringify(active));
    }
  }, [active]);
    ...
    try {
      ...
      if (savedActive && normalized.length) {
        try {
          ...
          if (found) {
            setActive({ ...found, conversationId: found.id });
          } else if (!active) {
            const firstUser = { ...normalized[0], conversationId: normalized[0].id };
            setActive(firstUser);
          }
        } catch {
  ...
  const debouncedSearch = useDebouncedCallback(async (q) => {
    ...
    try {
      ...
      const normalized = res.map(u => ({ ...normalizeUser(u), conversationId: u.id || u._id }));
      setUsers(normalized);
    } catch (e) {
  ...
  useEffect(() => {
    const newMessageHandler = (msg) => {
      ...
      if (document.visibilityState === 'visible' && (msg.from?.id || msg.from) !== user?.id) {
        setTimeout(() => {
          try {
            messageService.updateStatus(msg.id, 'read');
            socketService.updateMessageStatus(msg.id, 'read', active.conversationId);
          } catch (e) {
    ...
    const typingHandler = (data) => {
      ...
      if (data.conversationId === active.conversationId && data.userId !== user?.id) {
    ...
    const stopTypingHandler = (data) => {
      if (!data || !active) return;
      if (data.conversationId === active.conversationId) {
        const username = data.username || 'Someone';
        setTypingUsers(prev => prev.filter(u => u !== username));
      }
    };
    ...
    const presenceHandler = (data) => {
      ...
      setUsers(prev => prev.map(u => 
        u.id === data.userId 
          ? { ...u, status: data.status, lastSeen: data.lastSeen || data.timestamp } 
          : u
      ));
      
      // Update active user status if it's the same user
      if (active && active.id === data.userId) {
        setActive(prev => ({ ...prev, status: data.status, lastSeen: data.lastSeen || data.timestamp }));
      }
    };
    ...
    socketService.on('user_status_change', presenceHandler);
    socketService.on('message_sent', messageSentHandler);
    ...
    return () => {
      ...
      socketService.off('user_status_change', presenceHandler);
      socketService.off('message_sent', messageSentHandler);
    };
  }, [active, user?.id]);
  ...
  const selectUser = async (u) => {
    ...
    const conversationId = u.id; // For demo, use userId as conversationId
    ...
    try {
      ...
      const demoMessages = [
        { 
          id: 'd1', 
          from: 'them', 
          text: `Hello! I'm ${u.username || u.name}. How are you?`, 
          ts: Date.now() - 600000, 
          status: 'read' 
        },
        { 
        ...
        { 
          id: 'd3', 
          from: 'them', 
          text: 'Great to hear! This new chat interface looks amazing.', 
          ts: Date.now() - 300000, 
          status: 'read' 
        },
      ];
  ...
  const send = async () => {
    ...
    try {
      ...
      const socketSent = socketService.sendMessage({
        ...
        content: messageText,
        type: 'text',
        tempId: localId // For message_sent callback
      });
      
      if (socketSent) {
        // Socket succeeded, update status to sent
        setMessages(prev => prev.map(m => 
  ...
  const handleTyping = () => {
    if (active?.conversationId) {
      socketService.startTyping(active.conversationId);
    }
  };
  ...
  const StatusTicks = ({ status }) => {
    ...
    if (status === 'failed') return (
      <button onClick={send} title="Retry" className="hover:bg-gray-100 rounded p-1">
      ...
      <div className="topbar">
        ...
        <div className="status-pill">
          <span className={`w-1.5 h-1.5 rounded-full ${
            socketService.isSocketConnected() ? 'bg-green-500' : 'bg-yellow-500'
          }`} /> 
          ...
          <div className="chat-head">
            <div>
              <p className="text-sm font-semibold">
                {active ? (active.username || active.name) : 'Select a conversation'}
              </p>
              {active && (
                <p className="text-xs text-gray-500">
                  {active.status === 'online' 
                    ? 'Online' 
                    : active.lastSeen 
                      ? `Last seen ${new Date(active.lastSeen).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}` 
                      : (active.status || '')
                  }
                </p>
              )}
            ...
            {active && (
              <div className="status-pill">End-to-end encrypted</div>
            )}
          </div>
