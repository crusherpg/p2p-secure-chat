class SocketService {
  ...
  // Send message via socket (aligned with server contract)
  sendMessage(data) {
    if (this.socket && this.isConnected) {
      try {
        const payload = {
          ...
          type: data.type || 'text',
          encryptedContent: btoa(data.content || ''), // Simple base64 for demo
          iv: 'demo-iv',
          authTag: 'demo-tag',
  ...
  updateMessageStatus(messageId, status, conversationId) {
    if (this.socket && this.isConnected) {
      try {
        this.socket.emit('message_read', { messageId, conversationId, status });
      } catch (error) {
